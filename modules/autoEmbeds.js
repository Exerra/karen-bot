...
const autoEmbeds = (msg) => {
	...
	if (matchRegex(/https:\/\/open\.spotify\.com\/track\//, spotargs[0])) {
		...
		spotify
			.request(`https://api.spotify.com/v1/tracks/${/([0-Z]\w+)/.exec(spotargs[0])[1]}`)
			.then(function (data) {
				...
				embed.setAuthor('Spotify Song', 'https://cdn.exerra.xyz/png/companies/spotify/notext-transparent-2000x2000.png')
				...
				embed.addField('Release Date', `<t:${Date.parse(data.album.release_date) / 1000}:D>`, true)
				embed.setTimestamp()
		...
		spotify
			...
			.then(function (data) {
				...
				embed.setAuthor('Spotify Artist', 'https://cdn.exerra.xyz/png/companies/spotify/notext-transparent-2000x2000.png')
				embed.setColor(app.config.color)
		...
		spotify
			...
			.then(function (data) {
				...
				embed.setURL(data.external_urls.spotify)
				embed.setThumbnail(data.images[0].url)
				embed.setAuthor('Spotify Playlist', 'https://cdn.exerra.xyz/png/companies/spotify/notext-transparent-2000x2000.png')
				embed.setColor(app.config.color)
			...
			axios.get(`https://formulae.brew.sh/api/formula/${found[i]}.json`).then(res => {
				...
				embed
					.addFields(
						{
							name: "Downloads (30d)",
							value: data.analytics.install['30d'][Object.keys(data.analytics.install['30d'])[0]],
							inline: true
						},
						{
							name: "Downloads (365d)",
							value: data.analytics.install['365d'][Object.keys(data.analytics.install['365d'])[0]],
							inline: true
						}
					)

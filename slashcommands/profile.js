...
module.exports = {
  ...
  execute(client, interaction) {
    ...
    const sendProfile = (username, avatarURL, description, pronouns, birthday, createdAt, gender, country, rank, languages, email, website, twitter, id) => {
        ...
        if (!website.isEmpty()) embed.addField('Website', `[${website.replace(/(^\w+:|^)\/\//, '')}](${website} '${username}'s website')`, true); topLineFieldAmount++
        if (!email.isEmpty()) embed.addField('Email', `[${email}](mailto:${email})`, true); topLineFieldAmount++
        if (!twitter.isEmpty()) embed.addField('Twitter', `[@${twitter}](https://twitter.com/${twitter} '${username}'s twitter')`, true); topLineFieldAmount++
